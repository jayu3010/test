...
const BulkUnplannedPO = () => {
  ...
  const onFinish: SubmitHandler<FormData> = async (data: any) => {
    ...
    formData.append('isPlannedOrder', false);

    const modelClose = true;
    const res = await getShopData(
      formData,
      service?.API_URL?.productionOrder.import,
      modelClose
    );
    // if (res.data?.apiStatus === false) {
    //     setBulkPoFile(null);
    // }
  };
...
export default BulkUnplannedPO;
